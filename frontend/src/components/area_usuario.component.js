...
class AreaUSuario extends React.Component {
    ...
    validateData(e) {
        if (this.props.currentUser[0].Tipo == "user") {
            ...
            result_O3.then(valueO3 => {

                if (valueO3.data != false) {
                    latitud_ciudad = valueO3.data[0].LatitudCiudad;
                    longitud_ciudad = valueO3.data[0].LongitudCiudad;
                    //console.log("lat : " + latitud_ciudad + " long: " + longitud_ciudad);
                    valueO3.data.forEach(element => {
    ...
    crear_mapa(worst_points, points_O3, points_CO2, points_NO2, latitud_ciudad, longitud_ciudad) {
        ...
        if (worst_points.length > 0) {
            ...
            O3.setLatLngs(points_O3);
            //O3.setLatLngs(points_O3.map((point) => [point.Latitud,point.Longitud,point.Valor]));
